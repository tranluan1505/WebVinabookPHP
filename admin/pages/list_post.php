                                ...
                                <?php
                                 ...
                                 while($r=mysqli_fetch_assoc($rs))
                                 {
                                    ...
                                    <tr>
                                        ...
                                        <td><?=$count?></td>
                                        <td><?=$r['post_name']?></td>
                                        <td><img src="../<?php echo $r['post_image']?>" width="300px" heigh="300px"></td>
                                        <td><?=$r['post_decs']?></td>
                                        <td>Admin</td>
                                        <td class="clearfix">
                                                <a href="?page=form_edit_post&id=<?=$r['post_id']?>" title="Sửa" class="edit" style=margin:10px><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                <a href="?page=delete_post&id=<?=$r['post_id']?>" title="Xóa" class="delete" style=margin:10px><i class="fa fa-trash" aria-hidden="true" onclick=" return confirm ('Bạn chắc chắn muốn xóa tiếp không?')"></i></a>
                                        </td>
                                    </tr>
                                <?php
                                 }
